...
const spaceSubtitle = document.getElementById("spaceSubtitle");
...
const currentSiteCode = params.get("site") || "";
...
function isOwnerReservation(reservation) {
  return !!reservation && !!currentUser && currentUser.id === reservation.userId;
}

function canCancelReservation(reservation) {
  return !!reservation && !!currentUser && (currentUser.role === "admin" || currentUser.id === reservation.userId);
}
...
function buildReservationsMap(rows) {
  ...
  rows.forEach((reservation) => {
    if (!map[currentSiteCode][reservation.date]) {
      map[currentSiteCode][reservation.date] = [];
    }

    map[currentSiteCode][reservation.date].push({
      id: reservation.id,
...
async function loadSpaces() {
  ...
  spaces = data;
  currentSpace = spaces.find((space) => space.code === currentSiteCode) || null;
  ...
  spaceTitle.textContent = currentSpace.name;
  spaceSubtitle.textContent = `Calendario de ${currentSpace.name}`;
}
...
async function loadReservations() {
  ...
  const response = await fetch(`/api/reservations?site=${encodeURIComponent(currentSiteCode)}&month=${encodeURIComponent(month)}`);
  const data = await response.json();
  if (!response.ok) {
...
function renderSlots() {
  ...
  timeSlots.forEach((slot) => {
    ...
    const reservation = getReservationForSlot(currentSiteCode, isoDate, slot.start);
    ...
    if (isReserved) {
      ...
      if (cancellable) {
        ...
        slotEl.addEventListener("mouseenter", () => { button.textContent = "Cancelar"; });
        slotEl.addEventListener("mouseleave", () => { button.textContent = "Ocupado"; });
...
connectDropdown.addEventListener("click", (event) => { event.stopPropagation(); });
